...
export const Roadmap: React.FC = () => {
  return (
    <section id="roadmap" className="py-24 bg-space relative">
       {/* Background Glow */}
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-black text-white mb-6">Our Vision: The Dynamic Platform</h2>
          <p className="text-lg text-slate-400 max-w-2xl mx-auto">
            Where we're headed—building the future of personalized learning.
          </p>
        </div>

        <div className="bg-gradient-to-br from-[#111118] to-[#0a0a0f] border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl relative overflow-hidden">
          ...
          <h3 className="text-3xl md:text-4xl font-bold text-white mb-6">Dynamic Learning Platform</h3>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="flex items-center gap-3 p-4 bg-white/5 rounded-xl border border-white/5 hover:border-brand-purple/50 transition-colors group">
                <feat.icon size={20} className="text-slate-400 group-hover:text-brand-purple transition-colors" />
                <span className="text-slate-200 text-sm font-medium">{feat.text}</span>
              </div>
            ))}
          </div>

          <div className="mt-10 pt-8 border-t border-white/5 flex items-start gap-4 text-slate-400 text-sm italic">
             <Clock size={20} className="shrink-0 mt-0.5" />
             <p>The result? Students achieve the same learning outcomes in less time, with better retention—while faculty gain unprecedented insights into what works and what doesn't.</p>
          </div>
        </div>
      </div>
    </section>
